...
const queue = new FixedQueue(); // 所有的nextTick
...
function processTicksAndRejections() {
  ...
  do {
    ...
    // 可以看到这个任务与nextTick的耦合关系有点问题
    // 虽然退出的时候nextTick不会被push
    // 但是退出时一般还有task,而这些task，会导致事件循环不会立即终止，而导致 microtask 会被推入
    runMicrotasks(); // 执行 microtasks
...
function nextTick(callback) {
  ...
  if (process._exiting) // 是否退出，则不会push至nextTick
    return;
...
module.exports = {
  setupTaskQueue() {
    ...
    setTickCallback(processTicksAndRejections); // 将nextTick的执行权交至 C
    return {
      nextTick,
      runNextTicks
    };
